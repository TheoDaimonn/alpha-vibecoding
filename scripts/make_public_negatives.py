...
bank-branch address negatives (a bank address, nothing to mask). Famous persons
and bank addresses carry no spans at all: per the TZ they are not personal
data and must simply be skipped. Templates use cased forms of famous surnames
and random case variants to make detection register-invariant. Deterministic
under SEED.
"""
...
def make_resolver(person, person2, pii):
    def resolve(placeholder):
        if placeholder == "full_nom":
            return f"{person[0]} {person[1]}", None
        ...
        if placeholder == "fam2_gen":
            return decline(person2[1], "gen", person2[3]), None
        if placeholder.startswith("fam_"):
            case = placeholder.split("_", 1)[1]
            return decline(person[1], case, person[3]), None
        if placeholder == "bank_addr":
            return f"{pii['bank_city']}, {pii['bank_street']} {pii['bank_house']}", None
...
Row kinds:
- pure ({kinds['pure']}): famous person in a neutral context, nothing to mask (no spans).
- mixed ({kinds['mixed']}): famous person (no spans) + real PII (PERSON, CARD_NUMBER, INN,
  PHONE, EMAIL, BIRTH_DATE, CITY/STREET/HOUSE) that must still be masked.
- bank ({kinds['bank']}): bank branch address, nothing to mask (no spans).
...
"""
    ...
    for split, rows in rows_by_split.items():
        for text, entities in rows:
            if entities and any(e["label"] == "PERSON" for e in entities) and shown < 4:
                shown += 1
